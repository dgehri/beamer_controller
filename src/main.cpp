...
const unsigned long BUTTON_MAX_PRESS_DURATION = 20u;  // ms
...
const unsigned long RESUME_TIMEOUT = 500u;  // ms
...
State run_state_machine(State current_state)
{
    ...
    {
    case State::INIT: next_state = State::PREP; break;

    case State::PREP:
        switch (lift_state)
        {
        case LiftState::UP: next_state = State::IS_UP; break;
        case LiftState::DOWN: next_state = State::IS_DOWN; break;
        case LiftState::INTER: next_state = State::IS_INTERMEDIATE; break;
        case LiftState::UNKNOWN: next_state = State::PREP; break;
        }
        break;
    ...
    case State::PRE_DOWN:
        ...
        else if (now - last_state_change_time >= LIFT_LOWER_DELAY)
        {
            next_state = State::MOVE_DOWN;
        }
        else if (lift_state == LiftState::DOWN ||
    ...
    case State::IS_DOWN:
        ...
        else if (lift_state == LiftState::UP || lift_state == LiftState::INTER)
        {
            next_state = State::PREP;
        }
        else if (now - last_state_change_time >= IDLE_TIMEOUT)
    ...
    case State::IDLE:
        if (projector_on || button_pressed)
        {
    ...
    case State::STOP: next_state = State::PREP; break;
    }
    ...
    if (next_state != current_state || renter_state)
    {
        ...
        case State::IS_UP: Serial.println("IS_UP"); break;

        case State::IS_UP_AND_ON: Serial.println("IS_UP_AND_ON"); break;

        case State::IS_DOWN: Serial.println("IS_DOWN"); break;

        case State::IS_INTERMEDIATE: Serial.println("IS_INTERMEDIATE"); break;

        case State::PRE_DOWN: Serial.println("PRE_DOWN"); break;
...
void lower_column()
{
    modbus_send_command(0x0003, 0x0001);
}
void raise_column()
{
    modbus_send_command(0x0001, 0x0001);
}
void stop_column()
{
    modbus_send_command(0x0002, 0x0001);
}
...
LiftState get_lift_state()
{
    ...
    if (result == node.ku8MBSuccess)
    {
        ...
        {
        case 0b10: return LiftState::DOWN;
        case 0b11: return LiftState::INTER;
        case 0b01: return LiftState::UP;
        default: return LiftState::UNKNOWN;
        }
    }
    else
    {
...
void go_to_sleep()
{
    ...
    // enable watchdog timer for periodic wake up
